...
large Next.js/Supabase repo. Before them the pipeline could not reach a green
`ship` checkpoint on that project; after them it reports `ship pipeline ready
(all checkpoints validated)`.

### Fixed

- **`forge clean` flagged git-ignored files as "unmanaged scratch", and the `ship` hygiene gate hard-failed the whole pipeline on them.** On a real project `forge clean --check` proposed ~465 deletion candidates that were every one git-ignored — hundreds of another tool's `.playwright-mcp/console-*.log` files and sibling `.claude/worktrees/*` checkouts of concurrent sessions — because the manifest's `[scratch]` globs (`.playwright-mcp/*.log`, an unanchored `_*`) matched them and `clean` never consulted `.gitignore`. `checkVerify` (checkpoint 6) then failed with `FORGE-1300` and told the operator to run `forge clean --apply`, which would move all of those other tools'/sessions' working files to `.forge/trash/`. `forge clean` now skips git-ignored paths **outside `.forge/`** by default (it still tidies forge's own ignored scratch under `.forge/scratch/`, `.forge/cache/`, …), enumerated once via `git ls-files -o -i --exclude-standard --directory`. New `--include-ignored` flag restores the pre-1.10.6 behaviour.
- **Every non-`code` quality gate reported its artefact "not found / UNVERIFIED" on an agent-mode run.** `self-review-gate`, `spec-completeness-gate`, `adr-quality-gate`, `tdd-gate`, `security-hygiene-gate`, `qa-coverage-gate`, `manual-test-plan-gate` and `four-stage-testing-gate` all resolved the spec directory with `slugify(ctx.Description)` — but agent mode (and `forge ship <checkpoint> -n <slug>`) always sets `--name/-n`, and writes every artefact under `.forge/specs/<name>/`. The gates looked in a different, description-slugified directory and found nothing, so a whole pipeline's worth of real artefacts came back UNVERIFIED. All of them now resolve the directory via `HookContext.SpecName` (new `ctxSpecSlug` helper); `testingPipelineEvidencePath` likewise accepts an already-resolved slug.
- **Those same gates only looked for the one canonical filename.** Agent mode writes `test.md` / `test-stubs.md` / `code-plan.md`, not `tests.md` / `impl-notes.md`. Added a `readSpecArtefact` helper with per-checkpoint aliases (`arch.md`↔`adr.md`, `test.md`↔`tests.md`↔`test-stubs.md`, `code-plan.md`↔`impl-notes.md`, …) so a gate accepts whichever name the pipeline actually produced.
- **`self-review-gate` flagged legitimate `...` in artefacts as "placeholder/hedging".** Once the slug fix above made the gate actually read the files, a bare `strings.Contains(content, "...")` fired on prose elisions, range shorthands (`P1-01 … P1-09`) and `{ ... }` code snippets. Bare `...` is out of the contains-scan; an unfilled ellipsis placeholder is still caught by a per-line check (a line that is *only* dots, or a `key: ...` / `= ...` stub) that skips fenced code blocks.
- **`tdd-gate` only recognised Gherkin (`Given`/`When`/`Then`, `Scenario:`) or Go (`func Test`) as a "test scenario".** A Jest / Vitest / Jasmine / Mocha stub built from `it()` / `test()` / `describe()` blocks failed the gate. Those markers now count, and the gate reads `test-stubs.md` in preference to forge's thin `test.md` summary.
- **The `code` checkpoint reported "N modified file(s)" and status `ok` from unrelated `git status` noise** — docs, `.forge/` ledgers, another session's stray screenshots — the "✓ Code with zero code" shape. `checkCode` now counts only genuinely source-like changes (`countChangedSourceFiles`: source extensions, never under `.forge/`, `docs/`, `growth/`). When a code *plan* was written but no source changed, the checkpoint reports `warning — implement then rerun forge ship code` and appends "(N other working-tree change(s) not attributed to this checkpoint)" instead of a misleading green.
- **Test artefacts were always written to `<root>/tests`, which many JS/TS projects' runners never collect** (Jest `roots: ["src"]`, CRA, Next.js) — the files then showed up as "unreachable / will never run". `pickTestsDir` now prefers an existing already-collected directory (`src/test`, `src/__tests__`, …); `allTestArtifactsExist` / `missingTestArtifacts` check both it and the legacy `tests/`.
- **The Test checkpoint's reachability check ran against every `*_test`/`*.spec` file in the repo,** dragging in the project's deliberately-separate `tests/e2e/*`, `tests/staging/*`, `tests/smoke/*` suites and reporting them as orphans — noise that buried the one signal that mattered. It now checks only the artefacts forge scaffolded for the current slug.
- **`ship:test:integration` (and the `ship-test` prompt template) hard-coded "Jest + supertest".** On a repo with no `supertest` dependency and no single HTTP entrypoint the generated stub could not compile. The prompts now tell the model to follow the project's own integration-test convention and never import a package the project lacks.
- **`forge agent submit --file` failed on a path containing `..` or mixed separators** (`FORGE-6900 … cannot find the path specified`) even when the file existed. The path is now `filepath.FromSlash` + `Clean`ed and resolved against the working directory before reading.
