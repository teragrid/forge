...
`),
	...
	// run: run approved tests for a feature locally.
	runCmd := &cobra.Command{
		Use:   "run <feature>",
		Short: "Run approved tests for a feature locally across selected families.",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			r, err := projectRoot(root)
			if err != nil {
	...
	parent.AddCommand(createCmd, approveCmd, runCmd, ciCmd)
