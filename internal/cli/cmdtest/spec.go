...
// Two sub-subcommands:
//
//	forge test spec generate <feature>
//	    LLM-generates (or templates) a YAML test spec at
//	    .forge/specs/<feature>/spec.yml describing test cases, families,
//	    and expected behaviours.  Dry-run by default.
//
...
// newSpecCmd returns the `forge test spec` parent subcommand with one child:
// `generate`.  `forge test run --spec <path>` is the companion run command
// registered on the parent `forge test` command.
func newSpecCmd() *cobra.Command {
	specParent := &cobra.Command{
		Use:   "spec",
		Short: "Generate YAML test specs for features.",
		Long: strings.TrimSpace(`
forge test spec manages feature test specifications.

  generate <feature>   Template a YAML spec describing test cases, families, and
                       expected behaviours.  Edit the file, then run:
                         forge test run --spec .forge/specs/<feature>/spec.yml

Spec location: .forge/specs/<feature>/spec.yml (default)
`),
	...
	generateCmd := &cobra.Command{
		...
		Short: "Generate a YAML test spec for a feature (edit it, then run `forge test run --spec <path>`).",
	...
	generateCmd.Flags().StringVar(&genRoot, "root", "", "project root (default: cwd)")
	generateCmd.Flags().StringVar(&genSpecPath, "spec", "", "output path for the spec file (default: .forge/specs/<feature>/spec.yml)")
	generateCmd.Flags().StringVar(&genDescription, "description", "", "feature description injected into the spec")
	generateCmd.Flags().StringArrayVar(&genFamilies, "family", nil, "test family to include (repeatable; default: unit,integration,regression)")
	generateCmd.Flags().BoolVar(&genDryRun, "dry-run", true, "print template without writing the file")
	generateCmd.Flags().BoolVar(&genJSON, "json", false, "emit machine-readable JSON")

	specParent.AddCommand(generateCmd)
	return specParent
}
...
func renderSpecGenerateText(cmd *cobra.Command, r *SpecGenerateResult) {
	...
	fmt.Fprintf(w, "forge test spec generate (%s)\n", mode)
