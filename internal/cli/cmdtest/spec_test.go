...
# Generated by forge test spec generate auth
...
func TestNewSpecCmd_FlagsRegistered(t *testing.T) {
	...
	for _, sub := range cmd.Commands() {
		if sub.Name() == "generate" {
			hasGenerate = true
			// Verify key flags exist on generate.
			if f := sub.Flags().Lookup("dry-run"); f == nil {
				t.Error("generate subcommand missing --dry-run flag")
			}
			if f := sub.Flags().Lookup("spec"); f == nil {
				t.Error("generate subcommand missing --spec flag")
			}
		}
	}
	if !hasGenerate {
	...
	// 'run' must NOT be a child of spec — it lives at forge test run.
	for _, sub := range cmd.Commands() {
		if sub.Name() == "run" {
