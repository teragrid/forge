...
// Package cmdadopt implements `forge adopt` (DEV-M1-43, spec §4).
//
...
func init() {
	verbmeta.Register(verbmeta.Manifest{
		Verb:    "adopt",
		Summary: "Adopt an existing project into the Forge framework (DEV-M1-43, spec §4).",
		Inputs: []string{
			"--root <path>     — project root (default: cwd)",
			"--dry-run         — preview files without writing (default)",
			"--apply           — write framework files",
			"--skip <file>     — skip a specific file (repeatable)",
		...
		GatesTouched: []string{"§4 adopt", "§11.1.2 Developer Promises #1–#6"},
...
func New() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "forge adopt brings an existing project under Forge management by scaffolding\n" +
			...
			"  .forge/manifest         — hygiene / managed file registry\n" +
			"  .forge/hygiene.yml      — scratch/managed pattern manifest\n" +
			"  .forge/conventions.json — machine-readable convention registry\n" +
			"  AGENTS.md               — multi-AI agent context file\n" +
