...
func TestCheckArch_LLM_GeneratesArchDoc(t *testing.T) {
	...
	cp := checkArch(root, "add payment api", "", mockPipe(root, mock), false)
...
func TestCheckArch_ExistingArch_Idempotent(t *testing.T) {
	...
	cp := checkArch(root, "existing arch feature", "", mockPipe(root, mock), false)
...
func TestCheckArch_NoSpec_Warning(t *testing.T) {
	...
	cp := checkArch(root, "missing spec feature", "", mockPipe(root, mock), false)
...
func TestCheckArch_NoDescription_WithLLM_Warning(t *testing.T) {
	...
	cp := checkArch(root, "", "", mockPipe(root, mock), false)
...
func TestCheckArch_LLMError_StubWritten(t *testing.T) {
	...
	cp := checkArch(root, "payment feature", "", mockPipe(root, mock), false)
...
func TestCheckArch_NoLLM_StubWritten(t *testing.T) {
	...
	cp := checkArch(root, "new feature", "", nil, false)
...
func TestCheckArch_LLM_SupabaseRPCResponse(t *testing.T) {
	...
	cp := checkArch(root, "supabase user profile", "", mockPipe(root, mock), false)
