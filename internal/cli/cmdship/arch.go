...
func checkArch(root, description string, pipe *LLMPipe) Checkpoint {
	...
	if pipe != nil {
		archGenSystem := "You are a Solution Architect performing an architecture review. " +
			"Given the feature specification, produce a concise Architecture Decision Record (ADR) in Markdown. " +
		...
		generated, err := pipe.InvokeWithKnowledge(
			"ship:arch:generate", "",
			archGenSystem,
			fmt.Sprintf("Feature: %s\n\nSpec:\n%s", description, specMD),
			3000,
			"arch", "api-design", "dab",
		...
		case generated != "":
			...
			cp.Detail = fmt.Sprintf(
				"arch document + openapi.yaml generated by %s: .forge/specs/%s/",
