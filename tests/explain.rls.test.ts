...
describe('explain — RLS policies', () => {
  const anon    = createClient(SUPABASE_URL, ANON_KEY);
  const service = createClient(SUPABASE_URL, SERVICE_KEY);

  it('anon role cannot read explain rows', async () => {
    // TODO: replace table name + assertion after schema migration
    const { data, error } = await anon.from('explain').select('*');
    expect(error).not.toBeNull(); // RLS should block anon reads
  });

  it('service role can read all explain rows', async () => {
    const { error } = await service.from('explain').select('*');
